...
public class MentorService : IMentorService
{
    ...
    public async Task<PagedResponse<List<GetMentorDTO>>> GetMentorsAsync(MentorFilter filter)
    {
        ...
        {
            return new PagedResponse<List<GetMentorDTO>>(System.Net.HttpStatusCode.InternalServerError, dbEx.Message);
        }
        catch (Exception ex)
        {
    ...
    public async Task<Response<List<MentorWithGroup>>> GetMentorsThatHas()
    {
        try
        {
            ...
            var existing = await (from m in _context.Mentors
                                  let count = _context.MentorGroups.Count(e => e.GroupId == m.Id)
                                  select new MentorWithGroup
                                  {
                                      Mentor = m
                                  }).Where(e => e.Count > 2).ToListAsync();
            return new Response<List<MentorWithGroup>>(existing);
        }
        catch (Exception e)
        {
            return new Response<List<MentorWithGroup>>(System.Net.HttpStatusCode.InternalServerError, e.Message);
        }
    }
    ...
    public async Task<Response<List<GroupWithCountOfStudentDto>>> GetStudentWithCountOfStudentDtoAsync()
    {
        ...
        {
            return new Response<List<GroupWithCountOfStudentDto>>(System.Net.HttpStatusCode.InternalServerError, e.Message);
        }
    }
